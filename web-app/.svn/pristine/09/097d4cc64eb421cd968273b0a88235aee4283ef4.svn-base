...
public class ExportController {
	...
	public void exportSheets(HttpServletRequest req, HttpServletResponse res, 
		...
		String jsonStr1 = req.getParameter("data");
		String jsonStr = new String(jsonStr1.getBytes("ISO8859-1"), "UTF-8");
		...
		sheetData.add(list);
		HSSFWorkbook wb = genExcel(list,sheetName,ir.getFileds().split(","),ir.getFiledNames().split(","),ir.getHeadFiledNames().split(","),bodySpan,headSpan,unitName,sumColSpan);
	...
	private HSSFWorkbook genExcel(List<Map<String, String>> dataList,
			...
			String[] displayFields,String[] headFiledNames,int[][]bodySpan,String[][]headSpan,String unitName,int sumColSpan){
		...
		for (int j = 0; j < displayFields.length; j++) {
			...
			if(headrow<headSpan.length&&headcol<headSpan[headrow].length){
				...
				if(isSpan!=null){
					...
					if(isSpan.indexOf("r")>-1){//跨行
						...
						sheet.addMergedRegion(new CellRangeAddress(3,(3+t-1),j, j));
						cell.setCellValue(headFiledNames[j]);
					}else if(isSpan.indexOf("c")>-1){//跨列
						if(k==0){
							k=j;
						}
						int t=Integer.parseInt(isfiled);
						sheet.addMergedRegion(new CellRangeAddress(3,3,j,j+t-1));
						cell.setCellValue(headFiledNames[k]);
						k++;
					}else{
			...
			int kd=1250;
			if(k!=0){
				if(headFiledNames[k-1].length()>1){
					kd=kd*headFiledNames[k-1].length();
				}else{
					kd=2500;
				}
			}else{
				if(headFiledNames[k].length()>1){
					kd=kd*headFiledNames[k].length();
				}else{
					kd=2500;
				}
			}
		...
		for (int j = 0; j < displayFields.length; j++) {
			...
			if(headrow<headSpan.length&&headcol<headSpan[headrow].length){
				...
				if(isSpan!=null){
					...
					if(isSpan.indexOf("r")>-1){//跨行
						int t=Integer.parseInt(isfiled);
						sheet.addMergedRegion(new CellRangeAddress(4,(4+t-1),j, j));
						cell.setCellValue(displayFields[j]);
					}else if(isSpan.indexOf("c")>-1){//跨列
						int t=Integer.parseInt(isfiled);
						sheet.addMergedRegion(new CellRangeAddress(4,4,j,j+t-1));
						cell.setCellValue(displayFields[j]);
					}else{
						cell.setCellValue(isSpan);//显示字典项
					}
...
//			qf.setWhereString("SQS='350200'");
	...
	public Map getTempReportDataFn(IReport ir,List<RowBase> dataList,Map<String,Map<String,String>> codeMap,
		...
		System.out.println(ir.getHeadFiledNames());
		//获取合并行列
		int[][] bodySpan=ir.mergeBody(TotalList,rowumnNames1);
		//获取合并行列
