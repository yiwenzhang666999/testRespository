...
define(['config'], function (config) {
    'use strict';
    var c_util = function () { }
    c_util.prototype.Observable = new ol.Observable();
    c_util.prototype.loadFlag=false;
    /////////////////////////////继承///////////////////////////
    /**
     * 拷贝继承
     */
    ...
    */
    ...
    c_util.prototype.getDate = function(fmt,date){
    ...
    c_util.prototype.showLoading = function () {
        if (this.loadFlag) {
            return;
        }
        $("#myAlert_").show();
    ...
    c_util.prototype.hideLoading = function () {
        this.loadFlag = false;
        $("#myAlert_").hide();
        $("#myAlert_mask").hide();
    }
    ...
    c_util.prototype.addEventListener = function (type, listen, opt) {
        if (typeof type !== 'string') {
            console.error("事件类型错误!(必须为字符串类型)");
            return;
        }
        if (typeof listen !== 'function') {
            console.error("事件执行函数类型错误！(必须为function类型)");
            return;
        }
        this.Observable.on(type, listen, opt);
    }
    c_util.prototype.removeEventListener = function (type) {
        if (typeof type !== 'string') {
            console.error("事件类型错误!(必须为字符串类型)");
            return;
        }
        this.Observable.removeEventListener(type);
    }
    c_util.prototype.dispatchEvent = function (type, opt) {
        if (typeof type !== 'string') {
            console.error("事件类型错误!(必须为字符串类型)");
            return;
        }
        this.Observable.dispatchEvent(type, opt)
    }
    return new c_util();
});