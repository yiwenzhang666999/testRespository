 ...
 *显示节点 
 * */
ol.interaction.Modify.prototype.showPoint_=function (){
	if (this.source_) {
		for(var z=0;z<this.pointArr_.length;z++){
			 this.source_.removeFeature(this.pointArr_[z]);
		}
		  ...
		  var features = this.source_.getFeatures();
		    for(var i=0;i<features.length;i++){
		    	...
		    	if(type=="Polygon"){
		    		var arr = features[i].getGeometry().getCoordinates();
		    		for(var j=0;j<arr.length;j++){
		    			var pointArr = arr[j];
		    			for(var z=0;z<pointArr.length-1;z++){
		    				var fea = new ol.Feature({geometry:new ol.geom.Point(pointArr[z])});
			    			fea.setStyle(style);
			    			this.pointArr_.push(fea);
		    			}
		    		}
		    		...
		    		for(var j=0;j<arr.length;j++){
		    			var ploArr = arr[j];
		    			for(var z=0;z<ploArr.length;z++){
		    				var pointArr = ploArr[z];
		    				for(var x=0;x<pointArr.length-1;x++){
		    					var fea = new ol.Feature({geometry:new ol.geom.Point(pointArr[x])});
			    				fea.setStyle(style);
			    				this.pointArr_.push(fea);
		    				}
		    			}
		    ...
		    this.source_.addFeatures(this.pointArr_);
		    var fs= new ol.Collection(this.pointArr_);
		    fs.forEach(this.addFeature_, this);
		    ol.events.listen(fs, ol.CollectionEventType.ADD,
		    	      this.handleFeatureAdd_, this);
		    	  ol.events.listen(fs, ol.CollectionEventType.REMOVE,
		    	      this.handleFeatureRemove_, this);
	  }
}
/**
...
ol.interaction.Modify.prototype.handleSourceAdd_ = function(event) {
  if (event.feature) {
	  var arr = this.features_.getArray();
	for(var i=0;i<arr.length;i++){
		if(arr[i].ol_uid==event.feature.ol_uid){
			return;
		}
	}
