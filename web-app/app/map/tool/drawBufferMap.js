 ...
 */
define([
	'map', 'style', 'treeHelper', 'util', 'loadTree'
], function (map_, style, treeHelper, util, loadTree) {
	'use strict';
	var layer;
							...
							mini.confirm("您未勾选" + (res.name ? res.name : '') + "的数据！是否勾选？", "温馨提示",
								function (action) {
									if (action == "ok") {
										...
										$(".close-btn").length > 0 ? $(".close-btn").click() : "";
									}
								}
							);
						}
							...
							function (action) {
								if (action == "ok") {
									...
									require(["bhdk"], function (bhdk) {
										...
										loadTree.loadTreeHasAdded(); // 将新创建数据添加在已选择面板中
		        						loadTree.loadUserLayerManagePanel(userObj.zqCode, true); // 重新加载政区
									});
								} else {
									$(".close-btn").length > 0 ? $(".close-btn").click() : "";
								}
    ...
    */
	...
	ol.interaction.Draw.handleEvent = function (event) {
		...
		if (this.freehand_ &&
			...
			/**
			 *扩展自由绘制点显示，当距离大于10时，添加一个点
			 *否则不添加点，防止自由绘制时点过多导致浏览器卡
			 *顿；
			 */
			if (!this.lastPixel) {
				this.lastPixel = [0, 0];
