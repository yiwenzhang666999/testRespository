...
//	地图工具测量点击
	...
	$(".zs-toolbar .btns a.tool-qh").click(function(e){
		...
		//by zzq 创建的是否吸附的隐藏
		if(!$(this).hasClass("selected") && !$(".is-buffer-snap").is(":hidden")){
			$(".is-buffer-snap").hide();
		}else if($(this).hasClass("selected")&&$(".tool-cj").hasClass("selected")){
			$(".is-buffer-snap").show();
		}	
...
//	2018/03/09 资源监管 底部工具栏 显示面板
	$(".zs-grid-tool .show-wgdh").click(function(e){
		e.stopPropagation();
		var tab = $(".zs-grid-tool .float-tab");
		var left = $(this).position().left;
//		tab.css("top", -(tab.outerHeight() + 8) + 'px').stop().fadeToggle('fast');
		tab.css("top", '53px').stop().fadeToggle('fast');
	});
	$(".zs-grid-tool .float-tab .header .close").click(function(e){
		e.stopPropagation();
		$(this).parents(".float-tab").fadeOut('fast');
	})
})